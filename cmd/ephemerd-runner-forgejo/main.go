	...
	app := &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Sources: cli.EnvVars("FORGEJO_INSTANCE_URL"),
			},
			&cli.StringFlag{
				Name:    "token",
				Usage:   "runner registration token",
				Sources: cli.EnvVars("FORGEJO_REG_TOKEN"),
			},
			&cli.StringFlag{
				Name:    "name",
				Usage:   "runner display name (default: hostname)",
				Sources: cli.EnvVars("FORGEJO_RUNNER_NAME"),
			},
			&cli.StringSliceFlag{
				Name:    "label",
				Usage:   "runs-on label (can be repeated)",
				Value:   []string{"ubuntu-latest"},
				Sources: cli.EnvVars("FORGEJO_RUNNER_LABELS"),
			},
		},
