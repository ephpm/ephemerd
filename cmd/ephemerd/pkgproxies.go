...
func startPkgProxies(cfg *config.Config, dataDir string, net *networking.Manager, log *slog.Logger) ([]proxies.CacheProxy, func()) {
	...
	for _, c := range candidates {
		...
		// Windows L2Bridge: containers reach this listener at the host's own
		// LAN address, where the host's inbound firewall default-denies it —
		// the VFP host /32 allow only governs the container's egress. Open a
		// scoped inbound allow for exactly this port from the container pool.
		// No-op on NAT, Linux and macOS. A failure is not fatal: the proxy is
		// then unreachable and the health probe below (which goes over
		// loopback) will still pass, so jobs may point at an unreachable
		// cache — hence the loud warning.
		port := c.port
		if err := net.OpenHostPort(port); err != nil {
			log.Warn("failed to open the package cache host port for the container pool; the proxy may be unreachable on L2Bridge nodes",
				"proxy", p.Name(), "port", port, "error", err)
		}
