...
// The provided context controls the lifetime of the tunnel — cancelling it
// will abort connection setup and close any established connections.
func Listen(ctx context.Context, options Options) (*Listener, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, setupURL, nil)
	if err != nil {
		cancel()
		return nil, fmt.Errorf("creating setup request: %s", err)
	}
