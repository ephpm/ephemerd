...
func TestGenerateSandboxProfile(t *testing.T) {
	...
	checks := []struct {
		...
		{"blocks sibling job reads", `(deny file-read* (subpath "/var/lib/ephemerd/native"))`},
		{"blocks sibling job writes", `(deny file-write* (subpath "/var/lib/ephemerd/native"))`},
		{"blocks SSH dir reads", `(deny file-read* (subpath`},
		{"blocks SSH dir writes", `(deny file-write* (subpath`},
		{"blocks config.toml reads", `(deny file-read* (literal "/var/lib/ephemerd/config.toml"))`},
		{"blocks config.toml writes", `(deny file-write* (literal "/var/lib/ephemerd/config.toml"))`},
		{"blocks ephemerd socket reads", `(deny file-read* (literal "/var/lib/ephemerd/ephemerd.sock"))`},
		{"blocks ephemerd socket writes", `(deny file-write* (literal "/var/lib/ephemerd/ephemerd.sock"))`},
		{"blocks VM dir reads", `(deny file-read* (subpath "/var/lib/ephemerd/vm"))`},
		{"blocks VM dir writes", `(deny file-write* (subpath "/var/lib/ephemerd/vm"))`},
		...
		{"re-allows job dir reads", `(allow file-read* (subpath "/tmp/test-native/job123"))`},
