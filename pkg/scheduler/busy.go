...
func (s *Scheduler) probeLocalBusy(ctx context.Context, rj *runningJob) (runnerbusy.State, string, error) {
	...
	case rj.nativeRunner != nil:
		p, ok := rj.nativeRunner.(interface {
			BusyState(context.Context) (runnerbusy.State, error)
		})
		if !ok {
			return runnerbusy.Unknown, "native-process", runnerbusy.ErrUnsupported
		}
		st, err := p.BusyState(ctx)
		return st, "native-process", err
