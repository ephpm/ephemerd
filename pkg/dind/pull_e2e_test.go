...
func TestImagePull_SharedNamespaceHitIsUsableFromJobNamespace(t *testing.T) {
	...
	// Everything below is what /containers/create needs and what the bug
	// made impossible.
	...
	// The handler must have gone to the registry to resolve the ref into the
	// job namespace instead of borrowing the shared namespace's handle.
	//
	// Layer/config GETs are expected to be zero here: containerd's default
	// bolt content sharing policy ("shared") lets the ingest short-circuit
	// on a digest that is already in the backing content store, so the
	// re-pull costs a resolve round-trip and an unpack, not bandwidth. That
	// is a containerd default rather than a guarantee, so it is logged, not
	// asserted.
	if manifestReqs == 0 {
		t.Error("registry was never contacted; the handler resolved the image " +
