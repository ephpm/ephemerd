...
func TestBindTranslation_RealContainerd(t *testing.T) {
	...
	// Empty rootfs path keeps the e2e on the snapshot-layer fallback so
	// it exercises the upperdir/lowerdir walk against a real overlayfs
	// snapshot. Production calls pass the readlink-resolved bundle
	// rootfs path (a real on-host mount of the merged overlay) —
	// covered by the in-VM workflow run, not by this hermetic e2e.
