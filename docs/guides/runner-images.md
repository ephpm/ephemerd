...
The Linux image supports multi-arch (amd64 + arm64) via `docker buildx`. Each image includes an entrypoint script that copies the cached dependencies into the workspace so `mage ci` runs without downloading anything. The Go module cache is also pre-warmed in the image -- `go mod download` runs at build time so all dependencies are already present. At runtime, the only work is copying the cached assets into the container and running `mage ci`. No network downloads, no compilation of tools -- just build and test.
